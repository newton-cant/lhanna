...
case 'hentai':
	memein = await kagApi.memeindo()
	buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/random2/hentai?apikey=73cea893c3a573384e52e0a6`)
...
case 'conselho':
 ...
 if (!isUser) return reply(mess.only.userB)
  ...
  case 'play':   
   play = body.slice(5)
   anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?apikey=apivinz&q=${play}`)  
   ...
   break
    ...
    case 'play2':   
   play = body.slice(6)
   anu = await fetchJson(`https://videfikri.com/api/ytplayv2/?query=${play}`)
   buffer = await getBuffer(anu.result.thumbnal)
...
case 'vid':
 ...
 case 'instvid':
   ...
   if (anu.error) return reply(anu.error) 
   lagu = await getBuffer(anu.result.data)
   Pin.sendMessage(from, lagu, video, {mimetype: 'video/mp4', quoted: mek})
   break
